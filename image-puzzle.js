...
var imagePuzzle = {
    ...
    timer: function () {
        ...
        document.getElementById('timerPanel').textContent = elapsedTime;
        timerFunction = setTimeout(imagePuzzle.timer, 1000);
    },
    setImage: function (images, gridSize = 4) {
        var percentage = 100 / (gridSize - 1);
        var image = images[Math.floor(Math.random() * images.length)];
        document.createElement('imgTitle').innerHTML = image.title; 
        helper.doc('actualImage').setAttribute('src', image.src);
       /* helper.doc('myModal').setAttribute('src', image.src);*/
